...
const deleteDomaine = async (req, res) => {
    try {
        const id = req.params.id;
        const {data , error} = await supabase.from("domaines").delete().eq("domaine_id",id);
        if(error){
            return res.status(400).json({error: error.message})
        }
        res.json(data);
...
export { createDomaine, getAllDomaines, getDomaineById, updateDomaine, deleteDomaine , assignSoldeToWorkspaces}